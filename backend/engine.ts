...
export class ExecutionEngine {
  ...
  async execute<TInput, TOutput, TState>(params: {
    ...
    const context: ExecutionContext<TState> = {
      ...
      executionPath: [],
      stepResults: {},
      state: initialState ?? (workflow.stateSchema ? {} as TState : undefined as any),
    ...
    try {
      ...
      let currentOutput: any = validatedInput;
  ...
  private async executeStep<TInput, TOutput, TState>(
    step: Step<any, TInput, TOutput, TState>,
    ...
    try {
      ...
      const stepContext: StepExecutionContext<TInput, TState> = {
        ...
        getStepResult: <T extends Step<any, any, any, any>>(targetStep: T) => {
          const result = context.stepResults[targetStep.id];
          if (!result) {
            throw new Error(`Step ${targetStep.id} has not been executed yet`);
          }
          if (result.status !== 'success') {
            throw new Error(`Step ${targetStep.id} did not complete successfully`);
          }
          return result.output;
        },
        getInitData: () => context.inputData,
        runId: context.runId,
